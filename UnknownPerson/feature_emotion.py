...
def split_data_unknown(test_person_id, walk_nums_all, trace_nums_all, people_nums_all, person_nums, rand_seed = 42):
    ...
    trace_wlk_num = label_unique_tuples(people_nums_train_val, trace_nums_train_val, walk_nums_train_val)
    print(len(trace_wlk_num))
    ...
    for i in train_trace:
      idx = np.where(trace_wlk_num == i)[0]
...
def split_data_few_shot(test_person_id, walk_nums_all, trace_nums_all, people_nums_all, person_nums, ratio, rand_seed = 42):
    test_idx = np.empty((0,1))
    ...
    trace_wlk_num_all = label_unique_tuples(people_nums_all, walk_nums_all, trace_nums_all)
    trace_wlk_num_all = np.array(trace_wlk_num_all)
    test_idx = np.squeeze(test_idx)
    test_walk_nums = np.array([trace_wlk_num_all[int(i)] for i in test_idx])

    #training_test = random.sample(np.unique(test_walk_nums), sample_size)
    training_test_idx1 = np.random.choice(np.arange(len(np.unique(test_walk_nums))), size=int(ratio*len(np.unique(test_walk_nums))) , replace=False)
    training_test = np.unique(test_walk_nums)[training_test_idx1]
    
    training_test_idx = [np.where(trace_wlk_num_all == a)[0] for a in training_test]
    
    training_test_idx = np.concatenate(training_test_idx)
    train_val_idx = np.squeeze(train_val_idx)
    test_idx = np.setdiff1d(test_idx, training_test_idx)
    train_val_idx = np.concatenate((train_val_idx, training_test_idx))
    # trace_wlk_num = label_unique_tuples(people_nums_all, walk_nums_all, trace_nums_all)
